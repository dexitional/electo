...
export default function VoterLogin() {
  ...
  const authenticate = async (e:any) => {
      ...
      const { username } = form;
      try{
        const res = await verifyVoter({username});
        if(res.success){
        ...
        <div className="w-full mx-auto h-5 flex flex-row items-center justify-between py-3 bg-slate-50 border-b-[0.5px] border-solid border-gray-200/50">
            ...
            <div className="cursor-pointer">
                <Link href="https://electa.vercel.app">
                    <span className="py-0.5 px-4 bg-blue-900 font-bold italic leading-widest tracking-widest text-white">www.electa.app</span>
                </Link>
            </div>
        </div>
                ...
                <div className="w-full p-6 border bg-slate-50 border-gray-400/90 rounded-md shadow-lg shadow-red-900/30">
                    <form className="flex flex-col space-y-4">
                        ...
                        <div className="flex flex-col space-y-3">
                            ...
                            <input autoComplete='off' placeholder="Registration Number" type="text" name="username" onChange={onChange} className="py-2 px-4 w-full border text-gray-700 font-medium  placeholder:text-gray-500 placeholder:font-normal border-gray-400/90 rounded-[5px] outline-none" />
                        ...
                        <span className="mt-4 text-[10px] font-semibold text-center text-blue-900/90 hover:underline decoration-blue-900/90 cursor-default">Copyright &copy; DICTS-{new Date().getFullYear()}</span>
