...
export default function PublicList({page,setPage,setEid }:any) {
      ...
      <div className="flex flex-col sm:flex-wrap sm:flex-row space-x-6">
        ...
        <div key={row.tag} className="p-4 w-full sm:w-80 rounded border ">
            ...
            <div className="flex flex-col justify-center space-y-2">
              ...
              { row.allow_monitor == 1 ? (<button onClick={()=> gotoMonitor(row.id,row.name)} className="py-2 rounded text-blue-900 text-sm font-medium border border-blue-900">MONITOR THIS ELECTION</button>):null }
              { row.allow_result == 1 ? (<button onClick={()=> gotoResult(row.id,row.name)} className="py-2 rounded text-blue-900 text-sm font-medium border border-blue-900">VIEW ELECTION RESULTS</button>):null }
              {/*
              <button onClick={()=> gotoRegister(row.id,row.name)} className="py-2 rounded text-blue-900 text-sm font-medium border border-blue-900">VIEW VOTERS REGISTER</button>
              <button onClick={()=> gotoVip(row.id,row.name)} className="py-2 rounded text-blue-900 text-sm font-medium border border-blue-900">GOTO STRONGROOM</button>
              */}
